...
def inp_check():
    return f"{os.getcwd().replace(DIRECTORY, '~$')}: "

def manage(inp, file=b''):
    ...
    if spl_inp[0] in ["crtdir", "rmvdir", "swapdir", "crtfile", "lsdir", "readfile", "rmvfile"]:
        ...
        if check_direct(DIRECTORY, abs_src_path):
            try:
        ...
        if (check_direct(DIRECTORY, abs_src_path) and check_direct(DIRECTORY, abs_dst_path)):
            try:
...
functions = {"crtdir":crtdir, "rmvdir":rmvdir, "swapdir":swapdir, "crtfile":crtfile, "lsdir":lsdir, "readfile":readfile, "rmvfile":rmvfile, "cpyfile":cpyfile, "movefile":movefile, "renamefile":renamefile,"send_to_server":send_to_server, "get_from_server":get_from_server}
